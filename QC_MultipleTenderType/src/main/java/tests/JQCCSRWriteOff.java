...
public class JQCCSRWriteOff extends QCStore {
	public static void writeoff(String SSN, String AppURL) throws InterruptedException {
	int lastrow = TestData.getLastRow("Writeoff");
				...
				for (int row = 2; row <= lastrow+1; row++) {
					...
					if (SSN.equals(RegSSN)) {
						...
						driver.switchTo().defaultContent();
						driver.switchTo().frame("mainFrame");
						wait.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//li[@id='903000']")));
						driver.findElement(locator(Jprop.getProperty("collections_link"))).click();
		...
		//Admin Login For Getting Encryption Key
						 ...
						 JAdminLoginForEncryption.getEncryption(driver,SSN, AppURL);
						 Thread.sleep(5000);
						 driver.switchTo().defaultContent();
						 driver.switchTo().frame("mainFrame");
						 driver.switchTo().frame("main");
