...
public class JQCClearDropdown extends QCStore {
	...
	{
		try{
			int lastrow=TestData.getLastRow((Jprop.getProperty("Clear")));
			String sheetName=Jprop.getProperty("Clear");
					   ...
					   driver.switchTo().defaultContent();
						...
						if(ESign_CollateralType.equalsIgnoreCase("CHECK"))
						{
							for( String winHandle1 : driver.getWindowHandles())
							{
							 ...
							 driver.switchTo().frame("mainFrame");
							 driver.switchTo().frame("main");
							 driver.findElement(locator(Jprop.getProperty("transactionList"))).sendKeys("Clear Check");
							 ...
							 Thread.sleep(3000);
							 if(driver.findElement(locator(Jprop.getProperty("clear_checkyes"))).isDisplayed())
							 	{						
							...
							else if(ESign_CollateralType.equalsIgnoreCase("ACH"))
							{
								for( String winHandle1 : driver.getWindowHandles())
								{
								 ...
								 driver.switchTo().frame("mainFrame");
								 driver.switchTo().frame("main");
								 driver.findElement(locator(Jprop.getProperty("transactionList"))).sendKeys("ACH Clear");
								 ...
								 Thread.sleep(2000);
								 if(driver.findElement(locator(Jprop.getProperty("success"))).isDisplayed())
								 	{						
									 test.log(LogStatus.PASS, ESign_CollateralType+ "Clear dropdown from CSR is successfull");
									 test.log(LogStatus.PASS, "********************************************** ");								 
								 	}
