		...
		try {
			...
			for (int row = 2; row <= lastrow; row++) {
				...
				if (SSN.equals(RegSSN)) {
					...
					// login has started", ExtentColor.BLUE));

					// test.log(LogStatus.INFO, MarkupHelper.createLabel("Opened
					// the CSR URL " +AppURL, ExtentColor.BLUE));
					...
					/*
					 * try { driver.findElement(By.id("overridelink")).click();
					 * Thread.sleep(3000);
					 * 
			...
			// test.log(LogStatus.FAIL, MarkupHelper.createLabel("CSR login is
	...
	public static void logout(String SSN, String AppURL) {
		...
		try {
			...
			// if(driver.getTitle().contains("Login"))
			{
				...
				// test.log(LogStatus.INFO, MarkupHelper.createLabel("loggged
				// out from the CSR Application", ExtentColor.BLUE));
				test.log(LogStatus.INFO, "loggged out from the CSR Application");

				driver.quit();
			}
			/*
			 * else { test.log(LogStatus.PASS, "Logout was unsuccessfull"); }
			 */

		}

		catch (Exception e) {
			// test.log(LogStatus.ERROR, MarkupHelper.createLabel("CSR logout is
			// failed", ExtentColor.RED));
		...
		try {
			...
			for (int row = 2; row <= lastrow; row++) {
				...
				if (SSN.equals(RegSSN)) {
					...
					// String AdminURL =
					// TestData.getCellData(sheetName,"AdminURL",row);

					// test.log(LogStatus.INFO, MarkupHelper.createLabel("Admin
					// Application is launched", ExtentColor.BLUE));
	...
	public static void adminLogout(WebDriver driver, String SSN, String AppURL) {
		...
		try {
			...
			if (driver.getTitle().contains("Login")) {
				test.log(LogStatus.PASS, "Logout is Successfully");
				// test.log(LogStatus.INFO, MarkupHelper.createLabel("loggged
				// out from the Admin Application", ExtentColor.BLUE));
