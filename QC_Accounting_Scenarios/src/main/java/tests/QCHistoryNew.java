...
public class QCHistoryNew extends QCStore {
	...
	public static void history(String SSN, String AppURL) throws Exception {
			...
			try {
				...
				for (int row = 2; row <= lastrow; row++) {
					...
					if (SSN.equals(RegSSN)) {
						...
						Thread.sleep(3000);
						test.log(LogStatus.INFO, "History Roll over transaction has started");
						...
						driver.findElement(By.xpath("/html/body/form[1]/table/tbody/tr/td/table/tbody/tr/td/table[2]/tbody/tr[8]/td[2]/table/tbody/tr/td/table/tbody/tr[6]/td[13]/input")).click();
						test.log(LogStatus.PASS, "Clicked on Go button for the Rollover transaction");
						Thread.sleep(5000);
						...
						String cust_doc_total_amount_rollover=driver.findElement(By.xpath("//*[@id='transactionDetailsTable']/tbody/tr["+n+"]/td[7]/font")).getText();
					
				
						//cust_doc_total_amount="606.67";
						test.log(LogStatus.PASS, "Transaction amount from History is "+cust_doc_total_amount_rollover);
						String Acust_doc_total_amount_rollover[]=cust_doc_total_amount_rollover.split("-");
						cust_doc_total_amount_rollover=Acust_doc_total_amount_rollover[1];
						if(cust_doc_total_amount_rollover.equalsIgnoreCase(cust_doc_total_amount))
						{
							test.log(LogStatus.PASS, cust_doc_total_amount_rollover+" Transaction amount from History is equal to Transaction amount from Rollover History"+cust_doc_total_amount_rollover);	
						}
						else
						{
							test.log(LogStatus.FAIL, cust_doc_total_amount_rollover+" Transaction amount from History is not equal to Transaction amount from Rollover History "+cust_doc_total_amount_rollover);	
				...
				test.log(LogStatus.INFO, "process is initiated again due to Application sync issue");
		...
		if (i == 3) {
			test.log(LogStatus.FAIL, "History rollover failed");
