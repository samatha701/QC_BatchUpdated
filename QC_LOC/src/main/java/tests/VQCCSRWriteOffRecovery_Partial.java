...
public class VQCCSRWriteOffRecovery_Partial extends QCStore {
		...
		try {
			...
			for (int row = 2; row <= lastrow; row++) {
				...
				if (SSN.equals(RegSSN)) {
					...
					Thread.sleep(500);
					driver.findElement(By.name("requestBean.tenderAmt")).sendKeys("400");
					test.log(LogStatus.PASS, "Tender amount is entered as 400");		
					Thread.sleep(500);
