	...
	public static void StepSame(String SSN,String AppURL) throws Exception{
		...
		{		
			...
			{	
				...
				Thread.sleep(5000);
				try{
					driver.findElement(By.name("requestBean.vehicleGrade")).sendKeys("A");
					test.log(LogStatus.PASS, "Vechile grade selected as: A");
				}
				catch(Exception e){
					//Normal flow	
					}
				...
				if(stepupmsg.contains("Step Same")){
					...
					if(ESign_CollateralType.equalsIgnoreCase("Debit card")){
						...
						test.log(LogStatus.PASS, "Disb type enterd as Cash");
						String laonamount=driver.findElement(By.id("refinanceLoanAmt")).getAttribute("value");
						test.log(LogStatus.PASS, "Loan amount is"+laonamount);
						WebElement e2=driver.findElement(By.name("requestBean.siilBean.disbAmtFirst"));
						e2.clear();
						e2.sendKeys(laonamount);
						test.log(LogStatus.PASS, "Entered Disb amount is  :"+laonamount);*/
