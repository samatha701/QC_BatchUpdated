...
public class QC_NewLoan_Veritec extends CSR{
...

	{
		...
		{ 
			...
			{ 
				...
				//Check_Count=TestData.getCellData(sheetName,"ESign_Checks",row);
				System.out.println(No_of_checks);
...

	{
	    ...
	    if(VeritecRule.contains("Veritec could not hit due to process date mismatch")||VeritecRule.contains("This is not a valid test SSN. Please contact the Clarity Integration Team for help.")){
		...
		}else if (ESign_CollateralType.equals("ACH"))
		...
		else if (ESign_CollateralType.equals("DEBIT CARD"))
...
//*************************&**************************************//
		...
		try {
			...
			//**************Date move to sys date function****************//
