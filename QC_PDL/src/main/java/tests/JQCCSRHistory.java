...
System.out.println(columnname+"_TxnHistory_FieldValue");
	 ...
	 //ACH collateral details from sheet
			 ...
			 //Validations
						...
						String loan_account_inf_max_term_date= driver.findElement(locator(Jprop.getProperty("loan_account_inf_max_term_date"))).getText();
...
//Loan paid to date summary
						...
						else
						{
							...
							if(loan_status.toLowerCase().trim().equalsIgnoreCase(loan_status_inf_loan_status.toLowerCase().trim()))
							{
							...
							if(product_type.toLowerCase().trim().equalsIgnoreCase(loan_account_inf_product_type.toLowerCase().trim()))
							{
							...
							if(courtesy_consent_status.toLowerCase().trim().equalsIgnoreCase(courtesy_inf_courtesy_status.toLowerCase().trim()))
							{
				...
				/*			if(total_paid_amountc.equalsIgnoreCase(loan_paid_summary_total_paid_amt))
							...
							if(ESign_CollateralType.equals("ACH"))
							{
								...
								if(ACH_transaction.toLowerCase().trim().equalsIgnoreCase(ach_history_ach_transaction.toLowerCase().trim()))
								{
